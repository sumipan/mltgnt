...
def _parse_hhmm(s: str) -> tuple[int, int]:
    parts = s.strip().split(":")
    if len(parts) != 2:
        raise ValueError(f"Not HH:MM format: {s!r}")
...
class ScheduleJob:
    ...
    interval_minutes: Optional[int] = None  # for interval mode (minutes)
    ...
    def from_dict(cls, raw: dict[str, Any], *, default_timezone: str = _DEFAULT_TIMEZONE) -> "ScheduleJob":
        ...
        if not isinstance(args, dict):
            raise ValueError(f"job {jid}: action_args must be a dict")
        ...
        if mode == "scheduled":
            if not every:
                raise ValueError(f"job {jid}: scheduled requires every_day_at")
            _parse_hhmm(str(every))
        elif mode == "interval":
            if not interval_min or int(interval_min) <= 0:
                raise ValueError(f"job {jid}: interval requires interval_minutes > 0")
        elif mode == "fuzzy_window":
            if not ws or not we:
                raise ValueError(f"job {jid}: fuzzy_window requires window_start/end")
            ...
            if s_min > e_min:
                raise ValueError(
                    f"job {jid}: overnight windows are forbidden (window_start > window_end)"
                )
        elif mode == "chained":
            # chained: trigger when all depends_on complete. every_day_at is optional fallback
            if every:
                _parse_hhmm(str(every))
        else:
            raise ValueError(f"job {jid}: unknown mode: {mode}")

        if fuzzy_method not in ("hash", "random"):
            raise ValueError(f"job {jid}: fuzzy_method must be hash or random")
        if on_missed not in ("notify", "silent", "mark_done"):
            raise ValueError(f"job {jid}: on_window_missed is invalid")
        if notify not in ("silent", "slack_secretary", "slack_custom"):
            raise ValueError(f"job {jid}: notify is invalid")
        if notify == "slack_custom" and not slack_ch:
            raise ValueError(f"job {jid}: slack_custom requires slack_channel")
        ...
        if every_week_on and every_week_on not in DAY_NAMES:
            raise ValueError(
                f"job {jid}: every_week_on must be one of {DAY_NAMES}: {every_week_on!r}"
            )
    ...
    def window_minutes(self) -> tuple[int, int, int]:
        """Returns: (start_min, end_min, span_minutes closed interval)."""
