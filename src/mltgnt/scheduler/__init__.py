...
class PersonaScheduler:
    ...
    def _record_to_memory(
        ...
        try:
            ...
            _log.error("メモリ記録失敗 %s: %s", job.id, e)
    ...
    def _maybe_handle_missed_fuzzy(
        ...
        if job.on_window_missed == "notify":
            ...
            _log.debug("missed (silent): %s %s", job.id, d.isoformat())
        ...
        def runner() -> None:
            ...
            try:
                ...
                if ok:
                    ...
                    _log.warning("失敗: %s: %s", job.id, msg)
    ...
    def loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                _log.error("tick 例外: %s", e, exc_info=True)
            time.sleep(1.0)
