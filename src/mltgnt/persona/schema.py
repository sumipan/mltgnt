...
def build_engine_command(engine: str, model: str, prompt: str) -> list[str]:
    """engine / model の組み合わせから実行コマンドリストを生成する。

    Args:
        engine: "gemini" | "claude" | "cursor"
        model:  モデル名。空文字列の場合は engine 側の既定に委ねる
        prompt: プロンプト文字列
    ...
    if engine == "gemini":
        ...
        if model:
            return ["agent", "--model", model, "-p", prompt, "--force"]
        return ["agent", "-p", prompt, "--force"]
    else:
        valid = sorted(VALID_ENGINES)
        raise ValueError(
            f"未知の engine: {engine!r}。有効な値: {valid}"
        )


def build_engine_command_stdin(engine: str, model: str) -> list[str]:
    """stdin からプロンプトを受け取る形式のコマンドリストを生成する。

    exec.md（``cmd < order.md | tee result.md``）や triage のように
    prompt をコマンドライン引数ではなく stdin / input で渡す用途向け。

    Returns:
        subprocess.run() に渡せるコマンドリスト（prompt 引数なし）
