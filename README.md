...
| agent | `AgentResult` | dataclass | `tool`, `args`, `raw_response`, `tool_trace=None`, `reflexion_count=0`, `plan=None` (`tool == "__reflexion_exhausted__"` when `max_reflexions` is exceeded) | `mltgnt.agent._runner` |
| agent | `AgentRunner` | class | `(*, llm_call, tool_executor, terminal_tools, max_iterations=3, max_iterations_fn=None, evaluator=None, retry_config=None, logger=None, audit_writer=None, classifier=None, history_mode="last_result", history_max_chars=24_000, plan=None, max_reflexions=None, step_hook=None)`; defaults keep the single-result loop. `history_mode="full_trace"` sends the whole numbered trace, `plan` is updated in place from `plan_update` in responses, `step_hook(entry)` runs after each trace append | `mltgnt.agent._runner` |
| agent | `Plan` / `PlanItem` | dataclass | `Plan(items)`; `apply(updates)`, `progress() -> (done, total)`. `PlanItem(id, title, depends=[], status="pending", note="")` | `mltgnt.agent.plan` |
| agent | `parse_plan` | function | `(raw) -> Plan`; parses `{"items": [{"id", "title", "depends"?}]}` (omitted `depends` = previous item); raises `ValueError` | `mltgnt.agent.plan` |
| agent | `build_plan_prompt` | function | `(prompt) -> str`; English request for the `parse_plan` JSON contract | `mltgnt.agent.plan` |
| agent | `DefaultReflexionEvaluator` | class | `(failure_markers=(), repeat_window=3)`; retries on `[ERROR]` results, failure markers, or a repeated `(tool, args)` in the last `repeat_window` calls | `mltgnt.agent.reflexion` |
